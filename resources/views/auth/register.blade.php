    ...
    <div class="login-page">
      ...
      <form class="form login-form" method="POST" action="{{ route('register') }}">
        ...
        <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
          ...
          {!! Form::text('last_name', null, ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Enter your last name']) !!}
          <small class="text-danger">{{ $errors->first('name') }}</small>
        </div>
        ...
        {{--<div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
          ...
          {!! Form::text('mobile', null, ['class' => 'form-control', 'placeholder' => 'eg: 03201234567']) !!}
